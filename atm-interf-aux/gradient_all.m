function [c, dim] = gradient_all (F, h, dim, noend)
%GRADIENT_ALL:  Gradient with matrix-valued spacing along a given dimension
% 
% See also: mygradient

    if (nargin < 2) || isempty(h),  h = 1;  end
    if (nargin < 3),  dim = [];  end
    if (nargin < 4),  noend = [];  end
    if isempty(noend),  noend = false;  end
    if noend
        [c, dim] = gradient_all_noend (F, h, dim);
        return;
    end
    ...
    if ~isempty(dim) || isequaln(siz2, siz)
        [c, dim] = gradient_all_slow (F, h, dim);
        return;
    end
    nh = prod(siz2);
    if (nh ~= 1) && (nh ~= siz(1))
...
%%
function [c, dim] = gradient_all_slow (F, h, dim)
    if (nargin < 2) || isempty(h),  h = 1;  end
    if (nargin < 3),  dim = [];  end
    if isvector(h)
